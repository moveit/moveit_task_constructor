...
struct const_castable
{
	...
	static PyObject* convert(const Source& x) { return bp::incref(bp::object(std::const_pointer_cast<T>(x)).ptr()); }

	// register type conversion
	const_castable() { bp::to_python_converter<Source, const_castable<T>>(); }
};
...
void Task_execute(Task& self, SolutionBasePtr& solution) {
	...
	moveit::planning_interface::MoveGroupInterface mgi(
	    solution->start()->scene()->getRobotModel()->getJointModelGroupNames()[0]);
...
void export_core() {
	...
	bp::class_<SolutionBase, SolutionBasePtr, boost::noncopyable>("Solution", bp::no_init)
	    ...
	    .add_property("comment",
	                  bp::make_function(&SolutionBase::comment, bp::return_value_policy<bp::copy_const_reference>()))
	...
	properties::class_<Stage, std::auto_ptr<Stage>, boost::noncopyable>("Stage", bp::no_init)
	    ...
	    .add_property("name", bp::make_function(&Stage::name, bp::return_value_policy<bp::copy_const_reference>()),
	                  &Stage::setName)
	...
	bp::class_<ContainerBase, std::auto_ptr<ContainerBase>, bp::bases<Stage>, boost::noncopyable>("ContainerBase",
	                                                                                              bp::no_init)
	...
	bp::class_<ParallelContainerBase, std::auto_ptr<ParallelContainerBase>, bp::bases<ContainerBase>,
	           boost::noncopyable>("ParallelContainerBase", bp::no_init);
