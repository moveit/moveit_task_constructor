...
planning_scene::PlanningScenePtr ModifyPlanningScene::apply(const planning_scene::PlanningSceneConstPtr &scene, bool invert) const
{
	planning_scene::PlanningScenePtr result = scene->diff();
	// attach/detach objects
	for (const auto &pair : attach_objects_)
		(const_cast<ModifyPlanningScene*>(this))->attachObjects(*result, pair, invert);

	// enable/disable collisions
	for (const auto &pairs : collision_matrix_edits_)
		(const_cast<ModifyPlanningScene*>(this))->setCollisions(*result, pairs, invert);
