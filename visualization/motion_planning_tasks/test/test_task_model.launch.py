...
def generate_test_description():
    test_task_model = GTest(path=[PathJoinSubstitution(
        [LaunchConfiguration('test_binary_dir'), 'moveit_task_constructor_visualization-test-task_model'])],
        output='screen')
...
class TestTerminatingProcessStops(unittest.TestCase):
    def test_gtest_run_complete(self, proc_info, test_task_model):
        proc_info.assertWaitForShutdown(process=test_task_model)
